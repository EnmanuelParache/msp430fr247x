#[doc = r"Register block"]
#[repr(C)]
pub struct RegisterBlock {
    #[doc = "0x00 - Port 5 Input"]
    pub p5in: P5IN,
    _reserved1: [u8; 0x01],
    #[doc = "0x02 - Port 5 Output"]
    pub p5out: P5OUT,
    _reserved2: [u8; 0x01],
    #[doc = "0x04 - Port 5 Direction"]
    pub p5dir: P5DIR,
    _reserved3: [u8; 0x01],
    #[doc = "0x06 - Port 5 Resistor Enable"]
    pub p5ren: P5REN,
    _reserved4: [u8; 0x03],
    #[doc = "0x0a - Port 5 Select 0"]
    pub p5sel0: P5SEL0,
    _reserved5: [u8; 0x01],
    #[doc = "0x0c - Port 5 Select 1"]
    pub p5sel1: P5SEL1,
    _reserved6: [u8; 0x01],
    #[doc = "0x0e - Port 5 Interrupt Vector Register"]
    pub p5iv: P5IV,
    _reserved7: [u8; 0x06],
    #[doc = "0x16 - Port 5 Complement Select"]
    pub p5selc: P5SELC,
    _reserved8: [u8; 0x01],
    #[doc = "0x18 - Port 5 Interrupt Edge Select"]
    pub p5ies: P5IES,
    _reserved9: [u8; 0x01],
    #[doc = "0x1a - Port 5 Interrupt Enable"]
    pub p5ie: P5IE,
    _reserved10: [u8; 0x01],
    #[doc = "0x1c - Port 5 Interrupt Flag"]
    pub p5ifg: P5IFG,
}
#[doc = "P5IN (rw) register accessor: an alias for `Reg<P5IN_SPEC>`"]
pub type P5IN = crate::Reg<p5in::P5IN_SPEC>;
#[doc = "Port 5 Input"]
pub mod p5in;
#[doc = "P5OUT (rw) register accessor: an alias for `Reg<P5OUT_SPEC>`"]
pub type P5OUT = crate::Reg<p5out::P5OUT_SPEC>;
#[doc = "Port 5 Output"]
pub mod p5out;
#[doc = "P5DIR (rw) register accessor: an alias for `Reg<P5DIR_SPEC>`"]
pub type P5DIR = crate::Reg<p5dir::P5DIR_SPEC>;
#[doc = "Port 5 Direction"]
pub mod p5dir;
#[doc = "P5REN (rw) register accessor: an alias for `Reg<P5REN_SPEC>`"]
pub type P5REN = crate::Reg<p5ren::P5REN_SPEC>;
#[doc = "Port 5 Resistor Enable"]
pub mod p5ren;
#[doc = "P5SEL0 (rw) register accessor: an alias for `Reg<P5SEL0_SPEC>`"]
pub type P5SEL0 = crate::Reg<p5sel0::P5SEL0_SPEC>;
#[doc = "Port 5 Select 0"]
pub mod p5sel0;
#[doc = "P5SEL1 (rw) register accessor: an alias for `Reg<P5SEL1_SPEC>`"]
pub type P5SEL1 = crate::Reg<p5sel1::P5SEL1_SPEC>;
#[doc = "Port 5 Select 1"]
pub mod p5sel1;
#[doc = "P5SELC (rw) register accessor: an alias for `Reg<P5SELC_SPEC>`"]
pub type P5SELC = crate::Reg<p5selc::P5SELC_SPEC>;
#[doc = "Port 5 Complement Select"]
pub mod p5selc;
#[doc = "P5IES (rw) register accessor: an alias for `Reg<P5IES_SPEC>`"]
pub type P5IES = crate::Reg<p5ies::P5IES_SPEC>;
#[doc = "Port 5 Interrupt Edge Select"]
pub mod p5ies;
#[doc = "P5IE (rw) register accessor: an alias for `Reg<P5IE_SPEC>`"]
pub type P5IE = crate::Reg<p5ie::P5IE_SPEC>;
#[doc = "Port 5 Interrupt Enable"]
pub mod p5ie;
#[doc = "P5IFG (rw) register accessor: an alias for `Reg<P5IFG_SPEC>`"]
pub type P5IFG = crate::Reg<p5ifg::P5IFG_SPEC>;
#[doc = "Port 5 Interrupt Flag"]
pub mod p5ifg;
#[doc = "P5IV (rw) register accessor: an alias for `Reg<P5IV_SPEC>`"]
pub type P5IV = crate::Reg<p5iv::P5IV_SPEC>;
#[doc = "Port 5 Interrupt Vector Register"]
pub mod p5iv;
